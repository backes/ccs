...
public class LTLFormulaPreprocessor {
	...
	public static String preprocessFormula(String formula) {
		...
		for( int i=0; i<parts.length-1; i++) { // -1 because last isn't checked
	...
	private static String replace( String formula, String pattern ) {
		...
		while( pos != -1 ) {
			...
			if( end == -1 ) return formula; // syntax error
			
			replaced.append(formula.substring(lastPos, pos));
			replaced.append("(");
			String set[] = formula.substring(pos+pattern.length()+1,end).split(",");
	...
	private static String processFairness(String formula) {
		...
		if( formula.startsWith(UFAIR+"(") ) {
			...
			return replaceDidCanSets("[]<> did("+ formula.substring(
					UFAIR.length() + 1, formula.length()-1) +")");
		}
		else if( formula.startsWith(SFAIR+"(") ) {
			if( !formula.endsWith(")") ) {
				return formula;
			}
			String set = formula.substring(SFAIR.length() + 1, formula.length()-1);
			return replaceDidCanSets("( []<> can("+ set +") -> []<> did(" + set +") )");
		}
		else if( formula.startsWith(WFAIR+"(") ) {
			if( !formula.endsWith(")") ) {
				return formula;
			}
			String set = formula.substring(WFAIR.length() + 1, formula.length()-1);
			return replaceDidCanSets("( <>[] can("+ set +") -> []<> did(" + set +") )");
		}
		return null;
	}
