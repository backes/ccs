...
public class WeakIdentityHashMap<K, V> implements Map<K, V> {
    ...
    private void expungeStaleEntries() {
        ...
        while ((e = (Entry<K, V>) this.queue.poll()) != null) {
            removing(e.value);
            final int index = e.hash & (table.length-1);
            Entry<K, V> prev = this.table[index];
            Entry<K, V> p = prev;
            while (p != null) {
        ...
        public Object[] toArray() {
            ...
            for (int i = 0; i < r.length; i++) {
                if (!it.hasNext()) { // fewer elements than expected
                    final Object[] tmp = new Object[i];
                    System.arraycopy(r, 0, tmp, 0, i);
                    return tmp;
                }
                r[i] = it.next();
            }
        ...
        private <T> T[] finishToArray(final T[] r, final Iterator<?> it) {
            ...
            while (it.hasNext()) {
                ...
                if (i == cap) {
                    ...
                    final T[] tmp = (T[]) java.lang.reflect.Array.newInstance(
                        a.getClass().getComponentType(), newCap);
                    System.arraycopy(a, 0, tmp, 0, newCap);
                    a = tmp;
                }
                a[i++] = (T) it.next();
            }
