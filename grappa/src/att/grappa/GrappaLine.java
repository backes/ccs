...
public class GrappaLine implements GrappaConstants, Cloneable, Shape {
     ...
     * (cubic) curve points. The winding rule for this path is defaulted (from
     * Grappa.windingRule).
     *
     * @param pts
     *            the <code>GrappaPoint</code> array used to describe the
     *            curve
     * @param type
     ...
     * (cubic) curve points as used by "dot". All of the initial geometry and
     * the winding rule for this path are defaulted.
     *
     * @param curve
    ...
    public boolean equals(Object obj) {
        if (obj == null || !(obj instanceof GrappaLine))
            return (false);
        final GrappaLine cmp = (GrappaLine) obj;
        if (cmp == this)
            return (true);
    ...
    private void addArrow(GeneralPath path, GeneralPath testpath,
        ...
        final double theta = Math.atan2((tip.y - shaft.y), (tip.x - shaft.x));
        final double half_width = width / 2.0;

        float x, y;

        path.lineTo(x =
                (float) (tip.x - (length * Math.cos(theta) - half_width
                        * Math.sin(theta))), y =
                (float) (tip.y - (length * Math.sin(theta) + half_width
                        * Math.cos(theta))));
        testpath.lineTo(x, y);
        path.lineTo(x =
                (float) (tip.x - (length * Math.cos(theta) + half_width
    ...
    private void updateLine(String curve) {
        ...
        if (curve.charAt(i) == 'e') {
            grpts[grpts.length - 1] =
                    new GrappaPoint(curve.substring(i + 2, j));
            for (i = ++j; i < len; i++) {
                if (curve.charAt(i) != ' ') {
                    break;
                }
    ...
    private void updateLine(GrappaPoint[] grpts, int type) {
        ...
        final GeneralPath grpath =
                new GeneralPath(windingRule, grpts.length + xpts + grpts.length
                        - 1);
        final GeneralPath grtestpath =
                new GeneralPath(windingRule, grpts.length + xpts + grpts.length
                        - 1);
        ...
        while (pts < grpts.length) {
            ...
            if (pts + 3 <= grpts.length) {
                grpath.curveTo(x = (float) grpts[pts].x, y =
                        (float) grpts[pts++].y, x2 = (float) grpts[pts].x, y2 =
                        (float) grpts[pts++].y, x3 = (float) grpts[pts].x, y3 =
                        (float) grpts[pts++].y);
                grtestpath
                    .curveTo(x - z, y - z, x2 - z, y2 - z, x3 - z, y3 - z);
            } else {
                lastWasLine = true;
                grpath.lineTo(x = (float) grpts[pts].x, y =
                        (float) grpts[pts++].y);
                grtestpath.lineTo(x - z, y - z);
            }
        }
    ...
    public Object clone() {
        try {
            final GrappaLine copy = (GrappaLine) super.clone();
            copy.path = (GeneralPath) path.clone();
            if (gpts != null) {
                copy.gpts = gpts.clone();
            }
            return copy;
