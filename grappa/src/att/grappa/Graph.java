...
public class Graph extends Subgraph {
    ...
    /**
     * When filtering a graph (see GrappaSupport.filterGraph) or whenever edge
     * names are desired as part of the output of a printGraph call, set this
     * value to true. The edge name will be included among edge attributes in
     * the print output as '__nAmE__'. filterGraph automatically handles setting
     * this variable, but users writing their own version of filterGraph will
     * need it.
     */
    ...
    // initialized below
    private static Hashtable<String, Attribute> sysdfltNodeAttributes = new Hashtable<String, Attribute>(8);
    private static Hashtable<String, Attribute> sysdfltEdgeAttributes = new Hashtable<String, Attribute>(7);
    private static Hashtable<String, Attribute> sysdfltGraphAttributes = new Hashtable<String, Attribute>(11);
    ...
    private static void putAttribute(Hashtable<String, Attribute> table, int type, String name,
            String value) {
        final Attribute attr = new Attribute(type, name, value);
        attr.clearChanged();
        table.put(name, attr);
    }
    ...
    public final java.awt.font.FontRenderContext REFCNTXT =
            new java.awt.font.FontRenderContext(IDENTXFRM,
    ...
    public boolean setSynchronizePaint(boolean sync) {
        final boolean oldSync = synchronizePaint;
    ...
    public Object getGrappaAttributeValue(String key)
        ...
        if (grattributes == null)
            return null;
        final Attribute attr = (grattributes.get(key));
        if (attr == null)
            return null;
    ...
    public Object setGrappaAttribute(String key, String value)
            throws IllegalArgumentException {
        if (grattributes == null) {
            grattributes = new Hashtable<String, Attribute>(4);
        }
        // the get also tests if key is null
        Attribute oldValue = getGrappaAttribute(key);
        if (oldValue == null) {
            if (!validGrappaAttributeKey(key)) {
                throw new IllegalArgumentException(GrappaConstants.PKG_UPLOW
                        + " attribute key must use \"" + GrappaConstants.PKG_LOWER
                        + "\" as a prefix");
            }
     ...
     * @see GrappaConstants#SUBGRAPH
     */
    public static Enumeration<String> getGlobalAttributeKeys(int type)
            throws IllegalArgumentException {
        switch (type) {
        case GrappaConstants.NODE:
     ...
     * @see GrappaConstants#SUBGRAPH
     */
    public static Enumeration<Attribute> getGlobalAttributePairs(int type)
            throws IllegalArgumentException {
        switch (type) {
        case GrappaConstants.NODE:
    ...
    Element addIdMapping(Element elem) {
        ...
        if (id2element == null) {
            id2element = new Hashtable<Long, Element>();
        }
        return id2element.put(elem.getIdKey(), elem);
    }
    ...
    static Long idMapKey(int type, int id) throws IllegalArgumentException {
        long value = (id);
        final int tval = (type & (GrappaConstants.NODE | GrappaConstants.EDGE | GrappaConstants.SUBGRAPH));
        if (tval == 0) {
            throw new IllegalArgumentException(
    ...
    static int idKeyType(Long idKey) {
        final long value = idKey.longValue();
    ...
    public String setToolTipText(String text) {
        final String oldTip = toolTipText;
        toolTipText = text;
        return (oldTip);
    }
     ...
     */
    public void reset() {
        final String graphName = getName();
        if (delete())
            initialize(graphName);
    }
    ...
    public boolean setEditable(boolean mode) {
        final boolean wasMode = editable;
        editable = mode;
        return wasMode;
    }
    ...
    public boolean setSelectable(boolean mode) {
        final boolean wasMode = selectable2;
        selectable2 = mode;
        return wasMode;
    }
    ...
    public boolean setMenuable(boolean mode) {
        final boolean wasMode = menuable;
        menuable = mode;
        return wasMode;
    }
    ...
    public PrintWriter setErrorWriter(PrintWriter errWriter) {
        final PrintWriter oldWriter = this.errWriter;
