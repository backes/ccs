...
public class GrappaNexus implements GrappaConstants, Cloneable, ImageObserver,
    ...
    private final int windingRule = Grappa.windingRule;
     ...
     * <I>pos</I>, <I>rotation</I>, <I>shape</I>, <I>sides</I>, <I>skew</I>
     * and <I>width</I> attributes are examined. For edges, the <I>pos</I>
     * attribute is examined. For subgraph, the bounding box is recomputed.
     */
    public void updateShape() {
        final long thisShapeUpdate = System.currentTimeMillis();
        ...
        case NODE:
            ...
            final GrappaPoint pos =
                    (GrappaPoint) node.getAttributeValue(POS_ATTR);
            final Double Width = (Double) node.getAttributeValue(WIDTH_ATTR);
            final Double Height = (Double) node.getAttributeValue(HEIGHT_ATTR);
            final Integer Type = (Integer) node.getAttributeValue(SHAPE_ATTR);

            final double width = PointsPerInch * Width.doubleValue();
            final double height = PointsPerInch * Height.doubleValue();
            final int type = Type.intValue();
            ...
            final Integer Peripheries =
                    (Integer) node.getAttributeValue(PERIPHERIES_ATTR);
            final Integer Sides = (Integer) node.getAttributeValue(SIDES_ATTR);
            final Double Distortion =
                    (Double) node.getAttributeValue(DISTORTION_ATTR);
            final Double Skew = (Double) node.getAttributeValue(SKEW_ATTR);
            final Double Orientation =
                    (Double) node.getAttributeValue(ORIENTATION_ATTR);
            final Double Rotation =
                    (Double) node.getAttributeValue(ROTATION_ATTR);

            final int peripheries =
                    Peripheries == null ? -1 : Peripheries.intValue();
            final int sides = Sides == null ? -1 : Sides.intValue();
            final double distortion =
                    Distortion == null ? 0 : Distortion.doubleValue();
            final double skew = Skew == null ? 0 : Skew.doubleValue();
            double orientation =
            ...
            case CUSTOM_SHAPE:
                final String custom =
                        (String) node.getAttributeValue(CUSTOM_ATTR);
                if (custom == null) {
                ...
                try {
                    ccustom =
                            custom_class.getConstructor(new Class[] {
                ...
                try {
                    if (Grappa.centerPointNodes) {
                        ...
                        shape =
                                (Shape) (ccustom.newInstance(new Object[] {
                                        node, new Double(pos.x),
                                        new Double(pos.y), new Double(width),
                                        new Double(height) }));
                    }
                } catch (final Exception e) {
            ...
            case BOX_SHAPE:
                if ((Distortion == null || distortion == 0)
                    ...
                    if (Grappa.centerPointNodes) {
                        shape =
                                new Rectangle2D.Double(pos.x - (width / 2.0),
                                        pos.y - (height / 2.0), width, height);
                    } else {
                        shape =
                                new Rectangle2D.Double(pos.x, pos.y, width,
                                        height);
                    }
                            ...
                            if (Grappa.centerPointNodes) {
                                path.append(new Rectangle2D.Double(
                                        (pos.x - width / 2.0)
                                                + (i * PERIPHERY_GAP),
                                        (pos.y - height / 2.0)
                                                + (i * PERIPHERY_GAP), width
                                                - (2 * i * PERIPHERY_GAP),
                                        height - (2 * i * PERIPHERY_GAP)),
                                    false);
                            } else {
                                path.append(new Rectangle2D.Double(pos.x
                                        + (i * PERIPHERY_GAP), pos.y
                                        + (i * PERIPHERY_GAP), width
                                        - (2 * i * PERIPHERY_GAP), height
                                        - (2 * i * PERIPHERY_GAP)), false);
                            }
                        }
                        shape = path;
                    }
                    ...
                    if (Grappa.centerPointNodes) {
                        shape =
                                new GrappaShape(shapeType, pos.x, pos.y, width,
                                        height, sides, peripheries, distortion,
                                        skew, orientation, style.rounded,
                                        style.diagonals, null);
                    } else {
                        shape =
                                new GrappaShape(shapeType, pos.x
                                        + (width / 2.0),
                                        pos.y + (height / 2.0), width, height,
                                        sides, peripheries, distortion, skew,
                                        orientation, style.rounded,
                                        style.diagonals, null);
                    }
                }
                break;
            case ROUNDEDBOX_SHAPE:
                if ((Distortion == null || distortion == 0)
                    ...
                    if (Grappa.centerPointNodes) {
                        shape =
                                new RoundRectangle2D.Double(pos.x
                                        - (width / 2.0),
                                        pos.y - (height / 2.0), width, height,
                                        arcWidthFactor * width, arcHeightFactor
                                                * height);
                    } else {
                        shape =
                                new RoundRectangle2D.Double(pos.x, pos.y,
            ...
            case OVAL_SHAPE:
                if ((Distortion == null || distortion == 0)
                    ...
                    if (Grappa.centerPointNodes) {
                        shape =
                                new Ellipse2D.Double(pos.x - (width / 2.0),
                                        pos.y - (height / 2.0), width, height);
                    } else {
                        shape =
                                new Ellipse2D.Double(pos.x, pos.y, width,
                                        height);
                    }
                    if (Peripheries != null && peripheries > 1) {
                        path = new GeneralPath(shape);
                        for (int i = 1; i < peripheries; i++) {
                            if (Grappa.centerPointNodes) {
                                ...
                                path.append(new Ellipse2D.Double(pos.x
                                        + (i * PERIPHERY_GAP), pos.y
                                        + (i * PERIPHERY_GAP), width
                                        - (2 * i * PERIPHERY_GAP), height
                                        - (2 * i * PERIPHERY_GAP)), false);
            ...
            case MSQUARE_SHAPE:
                ...
                if (Grappa.centerPointNodes) {
                    ...
                    shape =
                            new GrappaShape(shapeType, pos.x + (width / 2.0),
                                    pos.y + (height / 2.0), width, height,
                                    sides, peripheries, distortion, skew,
                                    orientation, style.rounded,
                                    style.diagonals, null);
            ...
            if (Rotation != null && rotation != 0 && shape != null) {
                ...
                if (Grappa.centerPointNodes)
                    shape =
                            AffineTransform.getRotateInstance(theta, pos.x,
        ...
        case EDGE:
            ...
            if ((shape = (Shape) edge.getAttributeValue(POS_ATTR)) == null) {
                final Integer attr_type =
                        (Integer) (edge.getAttributeValue(DIR_ATTR));

                edge.direction =
                        (attr_type != null
                                          ? attr_type.intValue()
                                          : (edge.getGraph().isDirected()
                                                                         ? GrappaLine.TAIL_ARROW_EDGE
                                                                         : GrappaLine.NONE_ARROW_EDGE));

                // create a default straight line connecting the two
                // node centers
                edge.setAttribute("pos", new GrappaLine(new GrappaPoint[] {
                        (GrappaPoint) (edge.getTail()
                            .getAttributeValue(POS_ATTR)),
                        (GrappaPoint) (edge.getHead()
                            .getAttributeValue(POS_ATTR)) }, edge.direction));
                shape = (Shape) edge.getAttributeValue(POS_ATTR);
            }
            break;
        case SUBGRAPH:
            final Subgraph subgraph = (Subgraph) element;
            shapeType = BOX_SHAPE;
            ...
            Element elem = null;
            final GraphEnumeration enm = subgraph.elements();
            ...
            final GrappaSize minSize =
                    (GrappaSize) element.getAttributeValue(MINSIZE_ATTR);
            if (minSize != null) {
                if (sgbox == null) {
                    sgbox =
                            new java.awt.geom.Rectangle2D.Double(0, 0, minSize
            ...
            final GrappaBox minBox =
                    (GrappaBox) element.getAttributeValue(MINBOX_ATTR);
            if (minBox != null) {
                if (sgbox == null) {
                    sgbox =
                            new java.awt.geom.Rectangle2D.Double(minBox.x,
    ...
    public void updateStyle() {
        final long thisStyleUpdate = System.currentTimeMillis();
    ...
    /**
     * Update the text information for the underlying element. The <I>fontcolor</I>,
     * <I>fontname</I>, <I>fontsize</I>, <I>fontstyle</I>, and <I>label</I>
     * attributes are examined. The <I>lp</I> attribute is also examined for
     * edges and subgraphs.
     */
    public void updateText() {
        ...
        final long thisTextUpdate = System.currentTimeMillis();
        ...
        if (element.isEdge()) {
            if ((headstr = (String) element.getAttributeValue(HEADLABEL_ATTR)) != null
                    && (attr = element.getLocalAttribute(HEADLP_ATTR)) != null) {
                headpt = (GrappaPoint) (attr.getValue());
                lcnt++;
                hasEdgeLabel = true;
            } else
                headstr = null;
            if ((tailstr = (String) element.getAttributeValue(TAILLABEL_ATTR)) != null
                    && (attr = element.getLocalAttribute(TAILLP_ATTR)) != null) {
        ...
        if (labelAttr != null || hasEdgeLabel) {
            if (element.isNode()
                ...
                labelAttr =
                        labelAttr.substring(1, labelAttr.length() - 1).trim();
            }

            if (hasEdgeLabel || labelAttr.length() > 0) {

                final String fontname =
                        (String) element.getAttributeValue(FONTNAME_ATTR);
                final Integer fontstyle =
                        (Integer) element.getAttributeValue(FONTSTYLE_ATTR);
                final Integer fontsize =
                        (Integer) element.getAttributeValue(FONTSIZE_ATTR);
                final Integer fontadj =
                        (Integer) (element.getGraph())
                            .getGrappaAttributeValue(GRAPPA_FONTSIZE_ADJUSTMENT_ATTR);

                // set font
                tfont =
                        new Font(fontname, fontstyle.intValue(), fontsize
                            .intValue()
                                + fontadj.intValue());
                    ...
                    if (labelAttr != null) {
                        if (Grappa.autoPositionNodeLabel && element.isNode()) {
                            ...
                            if (element.isSubgraph() && lbox != null) {
                                lps[0] =
                                        new GrappaPoint(
                                                lbox.getX() + lbox.getWidth()
                                                        / 2.0,
                                                (Grappa.labelGraphBottom ? lbox
                                                    .getMaxY() : lbox.getMinY()));
                ...
                for (int l = 0; l < labels.length; l++) {
                    if (labels[l] != null && lps[l] != null
                        ...
                        if (tpos == null) {
                            ...
                            offset = tpos.length;
                            final GrappaPoint[] p =
                                    new GrappaPoint[offset + lines];
                            final String[] s = new String[offset + lines];
                        ...
                        for (i = 0; i < array.length; i++) {
                            if (array[i] == '\\'
                                    ...
                                    && ((ch = array[i]) == 'l'
                                            || array[i] == 'r' || array[i] == 'n')) {
                                tstr[offset + lines] =
                                        new String(array, start, i - 1 - start);
                                bnds[lines] =
                                        tfont.getStringBounds(tstr[offset
                                                + lines],
                                            element.getGraph().REFCNTXT);
                                mtrc[lines] =
                                        tfont.getLineMetrics(tstr[offset
                                                + lines],
                                            element.getGraph().REFCNTXT);
                        ...
                        if (start < array.length) {
                            tstr[offset + lines] =
                                    new String(array, start, array.length
                                            - start);
                            bnds[lines] =
                                    tfont.getStringBounds(tstr[offset + lines],
                                        element.getGraph().REFCNTXT);
                            mtrc[lines] =
                                    tfont.getLineMetrics(tstr[offset + lines],
                                        element.getGraph().REFCNTXT);
                            if (bnds[lines].getWidth() > wdinfo)
                                wdinfo = bnds[lines].getWidth();
                            //htinfo += bnds[lines].getHeight();
                        ...
                        for (i = 0; i < bnds.length; i++) {
                            ...
                            tpos[offset + i] =
                                    new GrappaPoint(x, top + tfont.getSize()
                                            - 1);
     ...
     * <I>color</I> and <I>fontcolor</I> attributes are examined. For edges,
     * the <I>dir</I> attribute is examined.
     */
    public void updateDecoration() {
        final long thisDecorationUpdate = System.currentTimeMillis();
        ...
        if (element.isEdge() && shape != null && shape instanceof GrappaLine) {
            ...
            int dir = graph_dir;
            final Integer attr_type =
                    (Integer) (edge.getThisAttributeValue(DIR_ATTR));
            if (attr_type != null)
                dir = attr_type.intValue();

            edge.direction = dir;

            final GrappaLine gline = (GrappaLine) shape;
            boolean forward =
                    gline.startsNear((Point2D) (edge.getTail()
                        .getAttributeValue(POS_ATTR)));
    ...
    public void updateImage() {
        final long thisImageUpdate = System.currentTimeMillis();
        final String path = (String) (element.getAttributeValue(IMAGE_ATTR));
    ...
    public Object clone() {
        try {
            final GrappaNexus copy = (GrappaNexus) super.clone();
            if (shape != null) {
     ...
     *            observed in place of that passed via <I>obs</I> and the
     *            second element is the update time of this new Attribute.
     */
    public void update(java.util.Observable obs, Object arg) {
        ...
        if (arg instanceof Long) {
            ...
            final int attrHash = attr.getNameHash();
            final long thisUpdate = ((Long) arg).longValue() + 1L;
            ...
            case EDGE:
                if ((POS_HASH == attrHash && POS_ATTR.equals(attrName))) {
                    ...
                    .equals(attrName))
                        ...
                        || (HEADLABEL_HASH == attrHash && HEADLABEL_ATTR
                            .equals(attrName))
                        || (HEADLP_HASH == attrHash && HEADLP_ATTR
                            .equals(attrName))
                        || (TAILLABEL_HASH == attrHash && TAILLABEL_ATTR
                            .equals(attrName))
                        || (TAILLP_HASH == attrHash && TAILLP_ATTR
                            .equals(attrName))
                        || (FONTSIZE_HASH == attrHash && FONTSIZE_ATTR
                            .equals(attrName))
                        || (FONTNAME_HASH == attrHash && FONTNAME_ATTR
    ...
    void drawImage(java.awt.Graphics2D g2d) {
        ...
        if (image != null) {
            if (shape instanceof CustomRenderer) {
                ...
                final Rectangle sbox = shape.getBounds();
                final Shape clip = g2d.getClip();
                g2d.clip(shape);
                g2d.drawImage(image, sbox.x, sbox.y, sbox.width, sbox.height,
                    null);
                g2d.setClip(clip);
            }
        }
