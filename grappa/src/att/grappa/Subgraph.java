 ...
 * @see Graph
 */
public class Subgraph extends Element implements Comparator<Object> {
    ...
    void setName() {
        ...
        while (true) {
            name =
                    Subgraph.defaultNamePrefix + getId() + "_"
    ...
    public Object getNodeAttributeValue(String key) {
        ...
        if (nodeAttributes == null)
            return (null);
        if ((attr = (nodeAttributes.get(key))) == null)
            return (null);
    ...
    public Object setNodeAttribute(Attribute attr) {
        ...
        final Subgraph sg = getSubgraph();
        final Attribute prntAttr =
                (sg == null) ? null : sg.getNodeAttribute(attr.getName());
        if (attr != prntAttr) {
    ...
    public Object setNodeAttribute(String name, Object value) {
        ...
        final Subgraph sg = getSubgraph();
        final Attribute prntAttr =
                (sg == null) ? null : sg.getNodeAttribute(name);
        ...
        if (crntAttr == null || crntAttr == prntAttr) {
            ...
            nodeAttributes.put(name, (crntAttr =
                    new Attribute(NODE, name, value)));
        } else {
            oldValue = crntAttr.getValue();
            if (value == null) {
                if (prntAttr == null) {
    ...
    private void removeNodeAttribute(String name) {
        ...
        final Attribute attr = nodeAttributes.remove(name);
        if (attr == null)
            return;
        attr.setValue("");
        if (attr.hasChanged())
    ...
    public Object setEdgeAttribute(Attribute attr) {
        ...
        final Subgraph sg = getSubgraph();
        final Attribute prntAttr =
                (sg == null) ? null : sg.getEdgeAttribute(attr.getName());
        if (attr != prntAttr) {
            // it's not, so use the other method;
            // use getStringValue to make sure value is treated properly
            // when converted to an Object
    ...
    public Object setEdgeAttribute(String name, Object value) {
        ...
        final Subgraph sg = getSubgraph();
        final Attribute prntAttr =
                (sg == null) ? null : sg.getEdgeAttribute(name);
        //if(prntAttr != null && value != null ) {
        //if(name.equals(prntAttr.getName()) && value.equals(prntAttr.getValue())) {
        // it is, so call other form
        ...
        if (crntAttr == null || crntAttr == prntAttr) {
            ...
            edgeAttributes.put(name, (crntAttr =
                    new Attribute(EDGE, name, value)));
    ...
    private void removeEdgeAttribute(String name) {
        if (name == null || edgeAttributes == null)
            return;
        final Attribute attr = edgeAttributes.remove(name);
        if (attr == null)
            return;
        attr.setValue("");
        if (attr.hasChanged())
    ...
    public Object setAttribute(Attribute attr) {
        ...
        final Subgraph sg = getSubgraph();
        final Attribute prntAttr =
                (sg == null) ? null : sg.getLocalAttribute(attr.getName());
        if (attr != prntAttr) {
            // it's not, so use the other method;
            // use getStringValue to make sure value is treated properly
            // when converted to an Object
    ...
    public Object setAttribute(String name, Object value) {
        ...
        final Subgraph sg = getSubgraph();
        final Attribute prntAttr =
                (sg == null) ? null : sg.getLocalAttribute(name);
        //if(prntAttr != null && value != null ) {
        //if(name.equals(prntAttr.getName()) && value.equals(prntAttr.getValue())) {
        // it is, so call other form
        ...
        if (crntAttr == null || crntAttr == prntAttr) {
            ...
            attributes.put(name, (crntAttr =
                    new Attribute(SUBGRAPH, name, value)));
    ...
    public Object getEdgeAttributeValue(String key) {
        ...
        if (edgeAttributes == null)
            return (null);
        if ((attr = (edgeAttributes.get(key))) == null)
            return (null);
        return (attr.getValue());
    }
    ...
    public java.awt.geom.Rectangle2D getBoundingBox() {
        ...
        if (grappaNexus == null || (bbox = grappaNexus.bbox) == null) {
            ...
            Element elem = null;
            final GraphEnumeration enm = elements();
            while (enm.hasMoreElements()) {
    ...
    public java.awt.geom.Rectangle2D resetBoundingBox() {
        Element elem = null;
        final GraphEnumeration enm = elements(SUBGRAPH);
        while (enm.hasMoreElements()) {
            elem = enm.nextGraphElement();
    ...
    private void printDfltAttr(PrintWriter out,
        ...
        Attribute attr;
        int nbr = 0;
        final Enumeration<Attribute> attrs = dfltAttr.elements();
    ...
    private Element findElementInSubgraphByName(int type, String name) {
        ...
        case NODE:
            if (nodedict != null)
                elem = nodedict.get(name);
            break;
        case EDGE:
            if (edgedict != null)
                elem = edgedict.get(name);
            break;
        case SUBGRAPH:
            if (graphdict != null)
                elem = graphdict.get(name);
            break;
        }
        ...
        final Enumeration<Subgraph> enm = graphdict.elements();
        while (enm.hasMoreElements()) {
            if ((elem =
                    (enm.nextElement()).findElementInSubgraphByName(type, name)) != null) {
                return elem;
            }
        }
    ...
    public Element createElement(int type, Object[] info, Attribute[] attrs) {
        ...
        case NODE:
            ...
            final Node node = new Node(this, nodeName);
            if (attrs != null) {
        ...
        case EDGE:
            ...
            final Node head = (Node) info[0];
            final String headPort = (String) info[1];
            final Node tail = (Node) info[2];
            ...
            final Edge edge =
                    new Edge(this, tail, tailPort, head, headPort, key);
            if (attrs != null) {
                for (int i = 0; i < attrs.length; i++) {
        ...
        case SUBGRAPH:
            ...
            final Subgraph newSubg = new Subgraph(this, subgName);
            if (attrs != null) {
                for (int i = 0; i < attrs.length; i++) {
    ...
    public Node removeNode(String nodeName) {
        if (nodedict == null)
            return (null);
        return ((nodedict.remove(nodeName)));
    }
    ...
    public Edge removeEdge(String edgeName) {
        if (edgedict == null)
            return (null);
        return ((edgedict.remove(edgeName)));
    }
    ...
    public Subgraph removeSubgraph(String graphName) {
        if (graphdict == null)
            return (null);
        return ((graphdict.remove(graphName)));
    }
    ...
    public boolean setShowSubgraphLabels(boolean value) {
        final boolean oldValue = subgLabels;
        subgLabels = value;
        return (oldValue);
    }
    ...
    public boolean setShowNodeLabels(boolean value) {
        final boolean oldValue = nodeLabels;
        nodeLabels = value;
        return (oldValue);
    }
    ...
    public boolean setShowEdgeLabels(boolean value) {
        final boolean oldValue = edgeLabels;
        edgeLabels = value;
        return (oldValue);
    }
     ...
     */
    public boolean isLR() {
        final Attribute attr = getAttribute("rankdir");
    ...
    private final double PATCHEDGE = 2;
    private final double PATCHEDGE2 = 2.0 * PATCHEDGE;
    ...
    public void patchWork(java.awt.geom.Rectangle2D.Double r, boolean square,
        ...
        if (mode == 0) {
            ...
            String style;
            Attribute attr;
            final Enumeration<?> enm = elements(GrappaConstants.SUBGRAPH);
            while (enm.hasMoreElements()) {
            ...
            final float sgtot = countOfElements(GrappaConstants.SUBGRAPH) - 2;
            float nbr = 0;
            Subgraph sg;
            String style;
            Attribute attr;
            ...
            while (enm.hasMoreElements()) {
                ...
                if (sg != this) {
                    sg.setAttribute(COLOR_ATTR, java.awt.Color.getHSBColor(
                        (float) (0.05 + 0.9 * (nbr++ / sgtot)), (float) 1.0,
                        (float) 1.0));
                    attr = sg.getAttribute(STYLE_ATTR);
                    if (attr == null) {
    ...
    private double prepPatchWork(String attrname, int mode) {
        ...
        if (graphdict != null && graphdict.size() > 0) {
            if (attrname != null)
                sgPatches = new Element[graphdict.size()];
            n = 0;
            final Enumeration<Subgraph> graphEnm = graphdict.elements();
            while (graphEnm.hasMoreElements()) {
                sg = graphEnm.nextElement();
        ...
        if (attrname != null && nodedict != null && nodedict.size() > 0) {
            m = 0;
            n = 0;
            if (mode <= 0)
                elPatches = new Element[nodedict.size()];
            else if (sgPatches == null)
                elPatches = new Element[nodedict.size()];
            else {
                n = sgPatches.length;
                elPatches = new Element[n + nodedict.size()];
                System.arraycopy(sgPatches, 0, elPatches, 0, n);
                sgPatches = null;
            }
            final Enumeration<Node> nodeEnm = nodedict.elements();
            while (nodeEnm.hasMoreElements()) {
                el = nodeEnm.nextElement();
    ...
    private void compSqPatchWork(java.awt.geom.Rectangle2D.Double r, boolean top) {
        ...
        if (top)
            ...
            box =
                    new GrappaBox(r.getX() + PATCHEDGE, r.getY() + PATCHEDGE, r
                        .getWidth()
        ...
        if (sgPatches != null) {
            ...
            while (i < sgPatches.length) {
                ...
                if ((i + 1) < sgPatches.length) {
                    ...
                    for (;;) {
                        tsz = (sgPatches[j]).getPatchSize();
                        tot = psz + sz + tsz;
                        tfrac = tot / total;
                        if (dir > 1) {
                            nscore =
                                    score(box.getWidth() * sz / tot, tfrac
                                            * box.getHeight());
                        } else {
                            nscore =
                                    score(tfrac * box.getWidth(), box
                                        .getHeight()
                                            * sz / tot);
                        }
                        if (nscore <= pscore) {
                            if (dir > 1) {
                                pscore =
                                        score(box.getWidth() * tsz / tot, tfrac
                                                * box.getHeight());
                            } else {
                                pscore =
                                        score(tfrac * box.getWidth(), box
                        ...
                        for (; i < j; i++) {
                            ...
                            if (dir > 1) {
                                p =
                                        new GrappaBox(prv, previous, nxt =
                                                box.getWidth()
                                                        * el.getPatchSize()
                                                        / tot, next);
                            } else {
                    ...
                    if (dir > 1) {
                        p =
                                new GrappaBox(box.getX(), previous, box
                                    .getWidth(),
                                        (next = frac * box.getHeight()));
                    } else {
                        p =
                                new GrappaBox(previous, box.getY(), (next =
                                        frac * box.getWidth()), box.getHeight());
                    }
                    ((Subgraph) el).compSqPatchWork(p, false);
                    i++;
                }
                ...
                if ((i + 1) < elPatches.length) {
                    ...
                    for (;;) {
                        ...
                        for (; i < j; i++) {
                            ...
                            if (el instanceof Node) {
                                if (dir > 1) {
                                    el.setPatch(prv, previous, nxt =
                                            box.getWidth() * el.getPatchSize()
                                                    / tot, next);
                                } else {
                                    el.setPatch(previous, prv, next, nxt =
                                            box.getHeight() * el.getPatchSize()
                                                    / tot);
                                }
                                p = el.getPatch();
                    ...
                    if (el instanceof Node) {
                        if (dir > 1) {
                            ...
                            el.setPatch(previous, box.getY(), (next =
                                    frac * box.getWidth()), box.getHeight());
                        }
                        p = el.getPatch();
                        el.setAttribute(POS_ATTR, new GrappaPoint(p
                ...
                if (el instanceof Node) {
                    ...
                    if (dir > 1) {
                        ((Subgraph) el).compStdPatchWork(new GrappaBox(box
                            .getX(), previous, box.getWidth(), (next =
                                frac * box.getHeight())), false);
                    } else {
                        ((Subgraph) el).compStdPatchWork(
                            new GrappaBox(previous, box.getY(), (next =
                                    frac * box.getWidth()), box.getHeight()),
                            false);
                    }
    ...
    class Enumerator implements GraphEnumeration {
        ...
        Enumerator(int t) {
            ...
            if (enm.hasMoreElements()) {
                ...
                while (enm.hasMoreElements()) {
                    subEnum =
                            ((Subgraph) (enm.nextElement())).new Enumerator(
                                    types);
                    if (subEnum.hasMoreElements()) {
                        if (elem == null) {
                            elem = subEnum.nextElement();
                        }
                        break;
                    }
                }
            ...
            if (enm == null) {
                if ((types & NODE) != 0
                        && (enm = nodeElements()).hasMoreElements()) {
                    dictType = NODE;
                    if (elem == null) {
                        elem = (Element) enm.nextElement();
                    }
                } else if ((types & EDGE) != 0
                        && (enm = edgeElements()).hasMoreElements()) {
                    dictType = EDGE;
                    if (elem == null) {
                        elem = (Element) enm.nextElement();
                    }
    ...
    public Vector<Element> vectorOfElements(int types) {
        final Vector<Element> retVec = new Vector<Element>();
        int count = 0;
        if ((types & NODE) != 0 && nodedict != null) {
            count += nodedict.size();
        ...
        if (graphdict != null) {
            ...
            final Enumeration<Subgraph> elems = graphdict.elements();
            while (elems.hasMoreElements()) {
                ((elems.nextElement())).recurseVectorOfElements(types, retVec,
                    count);
            }
        }
        return (retVec);
    ...
    // used above
    void recurseVectorOfElements(int types, Vector<Element> retVec, int count) {
        if ((types & SUBGRAPH) != 0)
