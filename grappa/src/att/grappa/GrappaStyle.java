...
/**
 * This class translates and encapsulates information provided by the <I>style</I>
 * attribute.
 *
...
public class GrappaStyle implements GrappaConstants, Cloneable {
     ...
     * Line color default.
     */
    public final static Color STYLE_LINE_COLOR_DEFAULT =
            GrappaColor.getColor("black", null);
    /**
     * Line style default.
     */
    ...
    static BasicStroke defaultStroke =
            new BasicStroke(STYLE_LINE_WIDTH_DEFAULT, STYLE_CAP_DEFAULT,
                    STYLE_JOIN_DEFAULT, STYLE_MITER_LIMIT_DEFAULT,
                    STYLE_DASH_DEFAULT, STYLE_DASH_PHASE_DEFAULT);

    static String defaultStrokeString =
            generateStrokeString(STYLE_LINE_WIDTH_DEFAULT, STYLE_CAP_DEFAULT,
                STYLE_JOIN_DEFAULT, STYLE_MITER_LIMIT_DEFAULT,
                STYLE_DASH_DEFAULT, STYLE_DASH_PHASE_DEFAULT);

    private static Hashtable<String, Integer> styleTypes =
            new Hashtable<String, Integer>(20);

    private static Hashtable<String, BasicStroke> strokeCache =
            new Hashtable<String, BasicStroke>(4);
     ...
     * @param style
     *            the <code>String</code> that specifies the style info.
     *            format is: style1,style2(extra2),...,styleN.
     */
    public GrappaStyle(int type, String style) {
        if ((type & (GrappaConstants.NODE | GrappaConstants.EDGE
                | GrappaConstants.SUBGRAPH | GrappaConstants.SYSTEM)) != type) {
    ...
    public void updateStyle(String style) {
            ...
            if (c == '(') {
                ...
                if (in_parens) {
                    ...
                    case STYLE_DASH_PHASE:
                        try {
                            dash_phase =
                                    Float.valueOf(option_string).floatValue();
                        } catch (final NumberFormatException nfe) {
                            throw new RuntimeException(
                    ...
                    case STYLE_DIAGONALS:
                        diagonals =
                                Boolean.valueOf(option_string).booleanValue();
                        break;
                    case STYLE_ROUNDED:
        ...
        final String strokeString =
                generateStrokeString(line_width, cap, join, miter_limit, dash,
                    dash_phase);
        if ((stroke = strokeCache.get(strokeString)) == null) {
            stroke =
                    new BasicStroke(line_width, cap, join, miter_limit, dash,
                            dash_phase);
    ...
    private static String generateStrokeString(float lineWidth, int capType,
        ...
        final StringBuffer strokeStringBuffer = new StringBuffer();
    ...
    public String toAttributeString() {
        return (generateStyleString(line_color, line_style, line_width, cap,
            join, miter_limit, dash, dash_phase, rounded, diagonals, filled,
            invis, fixed_size, font_style, false, elementType));
    }
    ...
    public String toString() {
        return (generateStyleString(line_color, line_style, line_width, cap,
            join, miter_limit, dash, dash_phase, rounded, diagonals, filled,
            invis, fixed_size, font_style, true, elementType));
    }
    ...
    private static String generateStyleString(Color color, int lineStyle,
        ...
        if ((type > 0 && (type & GrappaConstants.NODE) == GrappaConstants.NODE)
        ...
        if ((type > 0 && (type & (GrappaConstants.NODE | GrappaConstants.SUBGRAPH)) != 0)
        ...
        if ((type > 0 && (type & (GrappaConstants.NODE | GrappaConstants.EDGE | GrappaConstants.SUBGRAPH)) != 0)
    ...
    public Object clone() {
        try {
            final GrappaStyle copy = (GrappaStyle) super.clone();
            copy.dash = getDash();
            return copy;
