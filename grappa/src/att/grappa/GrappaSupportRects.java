...
public class GrappaSupportRects implements GrappaConstants {
    ...
    protected static synchronized Object[] parseRecordInfo(Node node) {
        ...
        final TableField tableField =
                doParse(node, !node.getSubgraph().isLR(), true);
        ...
        final double width =
                ((Double) node.getAttributeValue(WIDTH_ATTR)).doubleValue()
                        * PointsPerInch;
        final double height =
                ((Double) node.getAttributeValue(HEIGHT_ATTR)).doubleValue()
                        * PointsPerInch;
        final Dimension sz =
                new Dimension((int) Math.round(width), (int) Math.round(height));
        tableField.resizeFields(sz);

        final GrappaPoint pos = (GrappaPoint) node.getAttributeValue(POS_ATTR);
    ...
    private static boolean emitFields(TableField tf, Object[] objs) {
        ...
        final int fc = tf.fieldCount();

        if (fc == 0) {
            final Rectangle rect = tf.getBounds();
            if (rbuf == null) {
            ...
            ((GrappaPoint[]) objs[1])[fields] =
                    new GrappaPoint(rect.getCenterX(), rect.getCenterY());
            fields++;
            return true;
        }
    ...
    private static TableField doParse(Node node, boolean LR, boolean topLevel) {
        ...
        final TableField rv = new TableField();
        rv.setLR(LR);
        rv.subfields(maxf);
        if (topLevel) {
            rv.setParent(null);
...
class TableField implements GrappaConstants {
    ...
    void setId(String id) {
        ...
        if (id == null)
            return;
        final char[] array = id.toCharArray();
        ...
        if (fc == 0) {
            ...
            for (int cnt = 0; cnt < fc; cnt++) {
                ...
                if (tf.isLR()) {
                    ...
                    dim.height =
                            (dim.height > dtmp.height) ? dim.height
                                                      : dtmp.height;
                } else {
                    dim.width =
                            (dim.width > dtmp.width) ? dim.width : dtmp.width;
    ...
    void resizeUpFields(TableField tf, Dimension sz) {
        ...
        final Dimension delta =
                new Dimension(sz.width - tf.getSize().width, sz.height
                        - tf.getSize().height);
        tf.setSize(sz);
        ...
        final Dimension newSz = delta;
        for (int cnt = 0; cnt < fc; cnt++) {
            tfield = tf.fieldAt(cnt);
            amt =
                    (int) Math.floor(((cnt + 1)) * incr)
                            - (int) Math.floor((cnt) * incr);
            if (tf.isLR()) {
    ...
    private void posFields(TableField tf, Point pos) {
        ...
        final Rectangle b1 = tf.getBounds();
        final Rectangle b2 = tf.getTopMost().getBounds();
        int tmpi;
    ...
    void setTextBounds(String str, Node node) {
        ...
        final String fontname =
                node.getAttribute(FONTNAME_ATTR).getStringValue().toLowerCase();
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            fwidth +=
                    (cwFont)
                            ? fontwidth[0]
                            : ((value >= 0 && value < fontwidth.length)
                                                                       ? fontwidth[value]
                                                                       : 0);
        }
        if (fwidth > xwidth)
            xwidth = fwidth;
        final int height =
                ((Integer) node.getAttributeValue(FONTSIZE_ATTR)).intValue();
        final int width = (int) Math.round(height * xwidth);
