 ...
 * Drechsler, <a href="http://www.research.att.com">Research @ AT&T Labs</a>
 */
...

{
    ...
    private static final short ctype[] =
            { CN, CN, CN, CN, CN, CN, CN, CN, CN, CN | WS, CN | WS, CN | WS,
                    CN | WS, CN | WS, CN, CN, CN, CN, CN, CN, CN, CN, CN, CN,
                    CN, CN, CN, CN, CN, CN, CN, CN, WS | SP, PU, PU, PU, PU,
                    PU, PU, PU, PU, PU, PU, PU, PU, PU, PU, PU, DG | OD,
                    DG | OD, DG | OD, DG | OD, DG | OD, DG | OD, DG | OD,
                    DG | OD, DG, DG, PU, PU, PU, PU, PU, PU, PU, UC | HD,
                    UC | HD, UC | HD, UC | HD, UC | HD, UC | HD, UC, UC, UC,
                    UC, UC, UC, UC, UC, UC, UC, UC, UC, UC, UC, UC, UC, UC, UC,
                    UC, UC, PU, PU, PU, PU, PU, PU, LC | HD, LC | HD, LC | HD,
                    LC | HD, LC | HD, LC | HD, LC, LC, LC, LC, LC, LC, LC, LC,
                    LC, LC, LC, LC, LC, LC, LC, LC, LC, LC, LC, LC, PU, PU, PU,
                    PU, CN, };
    ...
    static String[] strsplit(String tuple) throws IllegalArgumentException {
        ...
        final StringTokenizer st = new StringTokenizer(tuple, ",", false);
    ...
    static double[] arrayForTuple(String tuple)
        ...
        final double[] array = new double[st.countTokens()];
    ...
    public static int xlateDirString(String direction) {
        if (direction != null) {
            final String dir = canonize(direction);
    ...
    public static int xlateFontStyleString(String fontstyle) {
        if (fontstyle != null) {
            final String style = canonize(fontstyle);
    ...
    public static Element findContainingElement(Subgraph subg, Point2D pt) {
        ...
        if (bb.contains(pt)) {
            ...
            while (enm.hasMoreElements()) {
                if ((subelem =
                        findContainingElement((Subgraph) (enm.nextElement()),
                            pt)) != null) {
                    elem = subelem;
                }
            }
     ...
     *         supplied box.
     */
    public static Vector<Element> findContainedElements(Subgraph subg,
            GrappaBox box) {
        ...
        if (box.contains(bb)) {
            return subg.vectorOfElements(GrappaConstants.SUBGRAPH
                    | GrappaConstants.NODE | GrappaConstants.EDGE);
        } else if (box.intersects(bb)) {
            Vector<Element> subelems = null;

            final Enumeration<Subgraph> subgraphEnm = subg.subgraphElements();
            while (subgraphEnm.hasMoreElements()) {
                if ((subelems =
                        findContainedElements((subgraphEnm.nextElement()), box)) != null) {
                    if (elems == null) {
                        elems = new Vector<Element>();
                    }
                    elems.addAll(subelems);
                }
            }
            final Enumeration<Edge> edgeEnm = subg.edgeElements();
            while (edgeEnm.hasMoreElements()) {
                final Edge edge = edgeEnm.nextElement();
                if ((grappaNexus = edge.grappaNexus) == null)
                    continue;
            ...
            final Enumeration<Node> nodeEnm = subg.nodeElements();
            while (nodeEnm.hasMoreElements()) {
                final Node node = nodeEnm.nextElement();
                if ((grappaNexus = node.grappaNexus) == null)
                    continue;
                if (box.contains(grappaNexus.rawBounds2D())) {
                    if (elems == null)
    ...
    public static void setHighlight(Element elem, int mode, int setting) {
        ...
        if (mode == 0) {
            ...
            final boolean wasDelete =
                    ((elem.highlight & DELETION_MASK) == DELETION_MASK);
            elem.highlight = 0;
            if (wasDelete) {
                if (elem.isNode()) {
                    final Enumeration<?> enm = ((Node) elem).edgeElements();
                    while (enm.hasMoreElements()) {
                        ((Element) (enm.nextElement())).highlight = 0;
                    }
                } else if (elem.isSubgraph()) {
                    final Enumeration<?> enm = ((Subgraph) elem).elements();
                    while (enm.hasMoreElements()) {
                        ((Element) (enm.nextElement())).highlight = 0;
                    }
                }
            ...
            if ((mode & DELETION_MASK) == DELETION_MASK) {
                if (elem.isNode()) {
                    if ((elem.highlight & DELETION_MASK) == DELETION_MASK) {
                        final Enumeration<?> enm = ((Node) elem).edgeElements();
                        while (enm.hasMoreElements()) {
                            ((Element) (enm.nextElement())).highlight |=
                                    DELETION_MASK;
                        }
                    } else {
                        final Enumeration<?> enm = ((Node) elem).edgeElements();
     ...
     * @throws IOException if there is a communication problem with the connector
     */
    public static boolean filterGraph(Graph graph, Object connector) throws IOException {
    ...
    public static boolean filterGraph(Graph graph, Object connector,
        ...
        try {
            if (connector instanceof java.lang.Process) {
                ...
                toFilterRaw =
                        ((java.net.URLConnection) connector).getOutputStream();
            } else {
                return false;
            }
        ...
        try {
            if (connector instanceof java.lang.Process) {
                fromFilterRaw =
                        ((java.lang.Process) connector).getInputStream();
            } else if (connector instanceof java.net.URLConnection) {
                fromFilterRaw =
                        ((java.net.URLConnection) connector).getInputStream();
            } else {
                return false;
            }
        } catch (final IOException ioex) {
        ...
        final StringBuffer newGraph = new StringBuffer(content.length() + 128);
        try {
            String line = null;
        ...
        try {
            ...
            if (!Grappa.displayException(ex) && (ex instanceof IOException))
    ...
    public static boolean centerPanel(Point2D cpt, GrappaPanel gpanel) {
        ...
        final Object prnt = gpanel.getParent();
        ...
        final javax.swing.JViewport vport = (javax.swing.JViewport) prnt;
        final java.awt.Rectangle b = gpanel.getBounds();
        final java.awt.Dimension d = vport.getExtentSize();
        final AffineTransform transform = gpanel.getTransform();
        if (transform == null)
            return false;
        final Point2D p = transform.transform(cpt, null);

        Rectangle2D r =
                new Rectangle2D.Double(p.getX() + b.x - (d.width) / 2., p
                    .getY()
                        + b.y - (d.height) / 2., d.width, d.height);
