...
public class GrappaShape implements GrappaConstants, Cloneable, Shape {
     ...
     * @param extra
     *            used only with the <I>record</I> shape at present, it
    ...
    public GrappaShape(int type, double x, double y, double width,
        ...
        final double sectorAngle = 2.0 * Math.PI / sides;
        final double sideLength = Math.sin(sectorAngle / 2.0);
        ...
        final GrappaPoint Rpt = new GrappaPoint();
        final GrappaPoint Ppt = new GrappaPoint();
        final GrappaPoint Bpt = new GrappaPoint();
        int verts = 0;
        double sinX, cosX;

        final GrappaPoint[] rawVertices = new GrappaPoint[sides];
        ...
        for (int j = 0; j < peripheries; j++) {
            ...
            if ((rounded || diagonals) && type != OVAL_SHAPE
                    && type != POINT_SHAPE && j == (peripheries - 1)) {
                tooSmall = false;
                tverts = 0;
                Pt0 = rawVertices[0];
                for (i = 0; i < sides; i++) {
                    ...
                    if (!diagonals) {
                        tmp2 = RBCURVE * tmp1;
                        tmpVertices[tverts++] =
                                new GrappaPoint(Pt0.x + tmp2 * (Pt1.x - Pt0.x),
                                        Pt0.y + tmp2 * (Pt1.y - Pt0.y));
                    }
                    tmpVertices[tverts++] =
                            new GrappaPoint(Pt0.x + tmp1 * (Pt1.x - Pt0.x),
        ...
        if (type == OVAL_SHAPE && diagonals) {
            ...
            Pt0 =
                    new GrappaPoint(width * CIRCLE_XDIAG / 2.0, height
                            * CIRCLE_YDIAG / 2.0);
            Ppt.x = x + Pt0.x;
            Ppt.y = y - Pt0.y;
    ...
    public Object clone() {
        try {
            final GrappaShape copy = (GrappaShape) super.clone();
