...
public abstract class GrappaColor {
    ...
    private static Hashtable<String, Color> colorTable =
            new Hashtable<String, Color>(660, 10);
    // given color, get name
    private static Hashtable<Color, String> colorLookUp =
            new Hashtable<Color, String>(660, 10);
    ...
    private static String canonColor(String name, float[] hsb) {
        ...
        if (hsb != null && allDigits && commas == 2 && dots <= 3) {
            ...
            try {
                for (int i = 0; i < 3; i++) {
                    hsb[i] =
                            Float.valueOf(name.substring(prev, commaSpots[i]))
                                .floatValue();
                    prev = commaSpots[i] + 1;
                }
